...
const cvaRoot = cva(['rounded-3xl p-1.2 relative bg-cBlack bg-opacity-[0.03] overflow-hidden'],{
    variants:{
        height:{
            sm:['h-10']
        }
...
const TaskTimer: FC<Props> = ({ category, status,fact,plan }) => {
    ...
    const calculateProgressPercent=(taskStatus:typeof status)=>{
        switch (status){
            case 'default':return fact/plan*100;
            default : return 100;
        }
    }
    ...
    return (
        <div className={cvaRoot()}>
            <div style={{width:`${taskProgress}%`}} className={cvaProgress({status})}></div>
            <div className={cvaTextBlock({status})}>
                <div className={cvaTextSection()}>
                    <p className={cvaTitle({align: "left"})}>{fact}{category=='time'?' ч':'%'}</p>
                    <p className={cvaDescription()}>{category=='time'?'всего':'факт'}</p>
                </div>
                <div className={cvaTextSection()}>
                    <p className={cvaTitle({align: "right"})}>{plan}{category=='time'?' ч':'%'}</p>
                    <p className={cvaDescription()}>{category=='time'?'оценка':'план'}</p>
                </div>
            </div>
        </div>
    );
